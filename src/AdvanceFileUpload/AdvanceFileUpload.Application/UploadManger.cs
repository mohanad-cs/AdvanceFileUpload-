...
namespace AdvanceFileUpload.Application
{
    ...
    public class UploadManger : IUploadManger
    {
        ...
        /// <param name="fileOperationService">The service for file operations.</param>
        /// <param name="logger">The logger for logging information.</param>
        /// <exception cref="ArgumentNullException">
        /// Thrown when any of the parameters are null.
        /// </exception>
        public UploadManger(IRepository<FileUploadSession> repository, IDomainEventPublisher domainEventPublisher, IFileValidator fileValidator, IChunkValidator chunkValidator, IOptions<UploadSetting> uploadSetting, IFileOperationService fileOperationService, ILogger<UploadManger> logger)
        {
        ...
        public async Task<CreateUploadSessionResponse> CreateUploadSessionAsync(CreateUploadSessionRequest request, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new Exception.ApplicationException("The File Name is not valid");
            }
            if (_fileValidator.ValidateFileExtension(request.FileExtension, _uploadSetting.AllowedExtensions))
            {
                throw new Exception.ApplicationException("The File Extension is not allowed");
            }
            if (_fileValidator.ValidateFileSize(request.FileSize, _uploadSetting.MaxFileSize))
            {
                throw new Exception.ApplicationException("The File Size is greater than the Maximum File Size");
            }
        ...
        public async Task<bool> CompleteUploadSessionAsync(Guid sessionId, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new Exception.ApplicationException("The Session is Not Completed");
            }
        ...
        public async Task<bool> UploadChunkAsync(UploadChunkRequest request, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new Exception.ApplicationException("The Chunk Index is Not Valid");
            }
            if (_chunkValidator.ValidateChunkSize(request.ChunkData.LongLength, _uploadSetting.MaxChunkSize))
            {
                throw new Exception.ApplicationException("The Chunk Size is greater than the Maximum Chunk Size");
            }
            cancellationToken.ThrowIfCancellationRequested();
            ...
            await _fileOperationService.SaveFileAsync(Path.GetFileName(outPutDir), request.ChunkData, outPutDir, cancellationToken);
